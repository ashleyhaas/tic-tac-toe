...
function checkForWin() {
  ...
  const boardArray = [null];

  for (let i = 1; i <= 9; i++) {
    const currentBoxHTML = document.querySelector("#box-" + i).innerHTML;
    boardArray.push(currentBoxHTML);
  }

  console.log(boardArray);

  function updateDOMAfterWin(winner) {
    setTimeout(function () {
      alert(winner + " Wins!");
    }, 200);
    isGameOver = true;
  ...
  function checkColumn(colNum) {
    const currentColumn = [];
    for (let i = colNum; i <= 9; i += 3) {
      const currentBoxHTML = boardArray[i];
      currentColumn.push(currentBoxHTML);
    }
    const firstItemInColumn = currentColumn[0];
    const hasWon = currentColumn.every(function (currentVal) {
      if (currentVal === "") {
        return false;
      } else {
        return currentVal === firstItemInColumn;
      }
    });
    return [hasWon, firstItemInColumn];
  }

  for (let i = 1; i <= 3; i++) {
    const currentColumnValue = checkColumn(i);
    const hasWon = currentColumnValue[0];
    const winner = currentColumnValue[1];
    if (hasWon) {
      updateDOMAfterWin(winner);
    }
  }
